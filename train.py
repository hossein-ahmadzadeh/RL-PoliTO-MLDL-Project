...
def main():
	...
	variances_episode_returns_window =  []  
...
# Agent's internal logs (mu, sigma, entropy) that are appended in get_action
	...
	for episode in range(args.n_episodes):
		...
		loss, returns_variance = agent.update_policy()
		losses.append(loss)
		agent_returns_variances_per_episode.append(returns_variance)
		...
		variances_episode_returns_window.append(current_variance_window)
		...
		if (episode + 1) % args.print_every == 0:
			...
			print(f'  Variance of Total Episode advantage (last {min(window_size, len(returns_window))} episodes): {variances_episode_returns_window[-1]:.2f}')
	...
	np.save(f"{analysis_dir}/variances_episode_returns_window.npy", np.array(variances_episode_returns_window))
